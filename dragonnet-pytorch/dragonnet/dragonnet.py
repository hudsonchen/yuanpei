...
class DragonNet:
    ...
    def create_dataloaders(self, x, y, t, valid_perc=None):
        ...
        if valid_perc:
            ...
            t_train = torch.from_numpy(t_train).reshape(-1, 1)
            t_test = torch.from_numpy(t_test).reshape(-1, 1)
    ...
    def validate_step(self):
        ...
        return torch.from_numpy(valid_loss).mean()
    ...
    def predict(self, x):
        ...
        Parameters
        ----------
        x: torch.from_numpy or numpy.array
            covariates

        Returns
        -------
        y0_pred: torch.from_numpy
            outcome under control
        y1_pred: torch.from_numpy
            outcome under treatment
        t_pred: torch.from_numpy
            predicted treatment
